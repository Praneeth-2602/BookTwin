...
export function ResultDisplay({ data, onOpenGraph, onOpenChat }: Props) {
  const { recommendation: r, pipeline_trace } = data;

  const goodreadsUrl = `https://www.goodreads.com/search?q=${encodeURIComponent(r.title + " " + r.author)}`;

  return (
    <div className="w-full group">
      <motion.div custom={0} variants={fadeUp} initial="hidden" animate="visible" className="mb-10 relative">
        ...
        <div className="flex gap-2 flex-wrap">
          {r.genres.map((g) => (
            <span key={g} className="text-[10px] px-2.5 py-1 rounded-full border border-[rgba(26,20,16,0.15)] text-[rgba(26,20,16,0.6)] tracking-wide uppercase">
              {g}
            </span>
          ))}
        </div>
        <div className="absolute top-0 right-0 w-16 h-16 rounded-full bg-[#1a1410] flex items-center justify-center shadow-lg transform rotate-[-5deg] group-hover:rotate-0 transition-transform duration-500">
          <div className="w-[56px] h-[56px] rounded-full border border-dashed border-[#f5f0e8] opacity-60 flex flex-col items-center justify-center">
            <span className="text-[#f5f0e8] font-mono text-xs font-bold leading-none">{r.match_score.replace('%', '').replace(' match','')}</span>
      ...
      <div className="flex flex-col md:flex-row gap-12">
        <motion.div custom={1} variants={fadeUp} initial="hidden" animate="visible" className="flex-1 flex flex-col gap-8">
          <Section label="Why you'll love it">
            <p className="font-serif text-[15px] leading-relaxed text-[#1a1410]">{r.why_you_will_love_it}</p>
          </Section>
          {r.mood_match && (
            <Section label="Mood match">
              <p className="font-serif text-[15px] leading-relaxed text-[#1a1410]">{r.mood_match}</p>
            </Section>
          )}
              ...
              <ul className="flex flex-col gap-2">
                {r.because_you_loved.map((reason, i) => (
                  <li key={i} className="flex gap-3 items-start">
                    ...
                    <span className="font-serif text-[14px] leading-relaxed text-[rgba(26,20,16,0.8)]">{reason}</span>
                  </li>
                ))}
              </ul>
            </Section>
          ...
          {r.caveat && (
            <div className="mt-4 pt-6 border-t border-[rgba(26,20,16,0.1)]">
              <div className="text-[10px] tracking-[0.15em] uppercase text-[rgba(26,20,16,0.4)] mb-2">Honest caveat</div>
              <p className="text-[12px] leading-relaxed text-[rgba(26,20,16,0.5)] italic">{r.caveat}</p>
            </div>
          )}
          <PaceEstimator bookTitle={r.title} />
        </motion.div>

        <motion.div custom={1} variants={fadeUp} initial="hidden" animate="visible" className="w-full md:w-[240px] shrink-0 flex gap-6">
          ...
          <div className="flex-1 flex flex-col">
            <div className="flex flex-col gap-2 mb-8">
              ...
              <ActionButton
                label="Generate character graph →"
                onClick={() => onOpenGraph(r.title)}
              />
              <ActionButton
                label="Chat with this book →"
                onClick={onOpenChat}
              />
            </div>
              ...
              <div className="flex flex-col gap-2">
                {pipeline_trace.map((t, i) => (
                  <div key={i} className="flex gap-2 items-start">
                    ...
                    <div className="text-[10px] font-mono text-[rgba(26,20,16,0.5)] leading-snug">{t.step}</div>
                  </div>
                ))}
              </div>
            </div>
...
function Section({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <div className="text-[10px] tracking-[0.18em] uppercase text-[#d4380d] mb-3 font-mono">{label}</div>
      {children}
    </div>
  );
}
